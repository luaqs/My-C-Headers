...
char *randIp()
{
    char *ip = malloc (999 + 1);
    sprintf(ip,"%d.%d.%d.%d",random_num(1, 126),random_num(1, 255),random_num(1, 255),random_num(1, 255));
    return ip;
}
//Returns A Random Hex
char *random_hex(int length)
{
    int i;
    char *src = malloc(999); // Sloppy Malloc Here Too !
    char *response = malloc (length * 2 + 1); // Sloppy Malloc ! You can fix it if you want !
    ...
    for (i = 0; i < length; i++)
    {
        src = randstrings[random_num(1, 285)];
        strcat(response, src);
    }
    free(src);
    return response;
